...
| # | 项 | 状态与说明 |
|---|---|---|
| 1 | **69shuba 住宅代理验证** | 数据中心 IP 被 Turnstile 风控（实测 `400030` 环境风控——与 UA/指纹无关）；代理配置为设计方向，未实现 |
| 2 | **服务端 TLS + HTTP/2/3** | 计划（未实现）。当前服务端纯 HTTP（TCP 监听，无 TLS/QUIC）；设计：`READER_APP_TLS_CERT`/`READER_APP_TLS_KEY` |
| 3 | **客户端 QUIC（HTTP/3）** | 计划（未实现）。Go `net/http` 默认 HTTP/1.1（TLS ALPN 可协商 HTTP/2）；QUIC 不启用 |
